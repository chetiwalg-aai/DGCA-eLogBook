...
Handles filling for rows where source === 'egcaexport'
*/
(function () {
  'use strict';
  const {
  ...
  // All dropdowns are now filled by matching the exact visible text from the
  // EGCA export raw data. No static value maps are used — the portal's
  // <option> text is expected to match the source table text exactly.
  ...
  async function fillRow(row, useAts = false) {
    console.log('[DGCA EGCA Filler] Processing row:', row);

    const { date, timeFrom, timeTo, dutyType } = row;
    ...
    if (raw.typeOfDuty === "Operation Duty(Control)") {
      ...
      // Unknown or unhandled type of duty
    }
  ...
  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg.type === 'START_FILLING_EGCA') {
  ...
  console.log('[DGCA Filler (EGCA Export)] Content script loaded on', window.location.href);
})();